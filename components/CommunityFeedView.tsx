...
const JobsFeedScreen: React.FC<{ user: any; onRequireLogin: () => void }> = ({ user, onRequireLogin }) => {
    ...
    return (
        <div className="bg-gray-50 dark:bg-gray-900 min-h-full pb-20">
            <div className="p-4 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 sticky top-0 z-30">
...
const CommunityNavBar: React.FC<{ currentView: string; onChangeView: (view: 'home' | 'direct' | 'explore' | 'profile' | 'jobs') => void; userAvatar?: string; hasUnreadMessages?: boolean; }> = ({ currentView, onChangeView, userAvatar, hasUnreadMessages }) => (
  <div className="sticky top-[70px] z-20 flex justify-center mb-0 w-full"> 
    <div className="bg-white/95 dark:bg-gray-800/95 backdrop-blur-md rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 w-full grid grid-cols-5 gap-1 items-center py-1 px-4 pointer-events-auto transition-all"> 
  ...
  const renderContent = () => {
    switch (internalView) {
      ...
      case 'direct':
        ...
        return <div className="w-full bg-white dark:bg-gray-900 pb-24">
            {/* Sticky Header for Direct Messages */}
            <div className="sticky top-0 z-30 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md px-4 pt-4 pb-2 border-b border-gray-100 dark:border-gray-800">
                <div className="relative">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                    <input type="text" placeholder="Buscar conversa..." className="w-full bg-gray-100 dark:bg-gray-800 rounded-xl py-3 pl-10 pr-4 text-sm outline-none focus:ring-2 focus:ring-[#1E5BFF]/50 transition-all" />
                </div>
            </div>
            <div className="px-4 py-2"><h3 className="font-bold text-gray-900 dark:text-white text-sm">Mensagens</h3></div>
            <div className="w-full flex-1">
                {MOCK_CHATS.map(chat => (<div key={chat.id} onClick={() => setSelectedChatId(chat.id)} className="w-full flex items-center gap-4 px-4 py-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50"><div className="relative flex-shrink-0"><img src={chat.avatar} className="w-12 h-12 rounded-full object-cover" /></div><div className="flex-1 min-w-0"><div className="flex justify-between items-center mb-0.5"><h4 className={`text-sm truncate pr-2 flex items-center gap-1 ${chat.unread ? 'font-bold' : ''}`}>{chat.user}</h4><span className="text-[10px] whitespace-nowrap text-gray-400">{chat.time}</span></div><p className="text-xs truncate">{chat.lastMsg}</p></div></div>))}
            </div>
        </div>;
      default: return null;
    }
