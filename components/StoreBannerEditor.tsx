...
const SIMPLE_COLORS = ['#1E5BFF', '#111827', '#64748B', '#334155', '#94A3B8', '#F59E0B', '#FFFFFF'];
...
const CATEGORY_IMAGES: Record<string, string[]> = {
  ...
  'Serviços': ['https://images.unsplash.com/photo-1581578731117-10d52b4d8051?q=80&w=800', 'https://images.unsplash.com/photo-1621905251189-08b45d6a269e?q=80&w=800'],
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar pb-40 bg-slate-950">
          ...
          {activeTab === 'background' && (
              <div className="space-y-6 animate-in fade-in duration-300">
                  ...
                  {bgSubTab === 'ready' && <div className="grid grid-cols-3 gap-3">{READY_BGS.map(url => <button key={url} onClick={() => setConfig({...config, bgType: 'ready', bgValue: url})} className={`aspect-square rounded-2xl overflow-hidden border-4 transition-all ${config.bgValue === url ? 'border-blue-600 scale-95 shadow-xl' : 'border-transparent opacity-60'}`}><img src={url} className="w-full h-full object-cover" alt="" /></button>)}</div>}
                  {bgSubTab === 'color' && <div className="grid grid-cols-4 gap-3">{SIMPLE_COLORS.map(color => <button key={color} onClick={() => setConfig({...config, bgType: 'color', bgValue: color})} className={`aspect-square rounded-2xl border-4 transition-all ${config.bgValue === color ? 'border-white scale-95 shadow-lg' : 'border-transparent opacity-80'}`} style={{ backgroundColor: color }} />)}</div>}
              </div>
          )}
          {activeTab === 'brand' && (
