...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, setActiveTab, userRole, viewMode }) => {
  ...
  if (viewMode !== 'ADM') {
    ...
    let centerId: string;
    if (!user) centerId = 'cashback_landing';
    else if (isMerchantView) centerId = 'merchant_qr_display';
    else centerId = 'scan_cashback';
    const centerLabel = isMerchantView ? 'QR Code' : 'Cashback';
    navItems.push({ id: centerId, icon: QrCode, label: centerLabel, isCenter: true });
  }
    ...
    if (item.isCenter) {
      return (
        <div className="relative w-16 flex justify-center -top-6">
          <button onClick={() => setActiveTab(item.id)} className="flex flex-col items-center group outline-none">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center shadow-2xl shadow-black/40 border-[6px] border-[#1E5BFF] active:scale-90 transition-all duration-200 ${isActive ? 'bg-[#1E5BFF]' : 'bg-white'}`}>
              <Icon className={`w-7 h-7 transition-colors ${isActive ? 'text-white' : 'text-[#1E5BFF]'}`} strokeWidth={2.5} />
            </div>
            <span className="text-[10px] font-black uppercase text-white mt-1 tracking-tight whitespace-nowrap drop-shadow-sm">{item.label}</span>
          </button>
        </div>
      );
    }
    return (
      <button onClick={() => setActiveTab(item.id)} className="w-full h-[60px] flex flex-col items-center justify-center gap-1 active:scale-95 transition-transform outline-none">
        <div className={`p-2 rounded-xl transition-all duration-300 ${isActive ? 'bg-white/20' : 'bg-transparent'}`}>
          <Icon className={`w-5 h-5 transition-colors ${isActive ? 'text-white fill-white' : 'text-white/60'}`} strokeWidth={isActive ? 0 : 2} />
        </div>
        <span className={`text-[9px] font-black uppercase transition-colors ${isActive ? 'text-white' : 'text-white/60'}`}>{item.label}</span>
      </button>
    );
  };
  return (
      ...
      <div className="flex items-end justify-around w-full px-2 h-full pb-2">
        {navItems.map((item) => {
          const isActive = activeTab === item.id || 
                          ...
                          (item.id === 'merchant_qr_display' && ['merchant_qr_display', 'merchant_onboarding', 'store_area'].includes(activeTab) && viewMode === 'Lojista') || 
                          (item.id === 'cashback_landing' && ['cashback_landing', 'scan_cashback', 'pay_cashback'].includes(activeTab) && viewMode === 'Visitante') || 
