...
export const MenuView: React.FC<MenuViewProps> = ({ 
  ...
  const handleCreateJob = () => {
    if (!hasSeenMerchantJobsVideo) {
      ...
      setHasSeenMerchantJobsVideo(true);
      updateProfile({ hasSeenMerchantJobsVideo: true });
    } else {
      onNavigate('merchant_jobs');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-24 animate-in fade-in duration-300">
      ...
      <div className="px-4 pb-5">
        ...
        {/* (B) CONDICIONAL: Notificações de Vagas (Cliente) OU Cadastrar Vaga (Lojista) */}
        {!isMerchant ? (
          ...
          <div 
            ...
            className="bg-white dark:bg-gray-800 rounded-[2rem] p-6 border border-gray-100 dark:border-gray-700 mb-6 shadow-sm cursor-pointer group hover:border-[#1E5BFF]/30 transition-colors"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-[#1E5BFF]/10 flex items-center justify-center text-[#1E5BFF]">
                  <Briefcase className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="font-bold text-gray-900 dark:text-white text-base leading-tight">Cadastrar vaga de emprego</h3>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400 font-medium mt-0.5">Encontre talentos na Freguesia.</p>
                </div>
              </div>
              <div className="w-8 h-8 rounded-full bg-gray-50 dark:bg-gray-700 flex items-center justify-center group-hover:bg-[#1E5BFF] group-hover:text-white transition-colors text-gray-400">
        ...
        {/* --- ÁREA EXCLUSIVA LOJISTA (UNIFICADA) --- */}
        {isMerchant && (
            <div className="mb-8">
               <h3 className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-4 ml-2">Gestão do Negócio</h3>
               
               <div className="grid grid-cols-2 gap-3">
                   {/* 1. Dashboard Principal */}
                   <button onClick={() => onNavigate('store_area')} className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col gap-3 items-start active:scale-95 transition-all">
                       ...
                       <div className="text-left">
                           <span className="text-sm font-bold text-gray-900 dark:text-white block">Dashboard</span>
                           <span className="text-[10px] text-gray-500 dark:text-gray-400">Visão Geral</span>
                       </div>
                   </button>

                   {/* 2. Terminal de Caixa */}
                   <button onClick={() => onNavigate('merchant_panel')} className="bg-gradient-to-br from-[#1E5BFF] to-[#1749CC] p-4 rounded-2xl shadow-lg shadow-blue-500/20 flex flex-col gap-3 items-start active:scale-95 transition-all text-white">
                       <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center border border-white/10">
                           <QrCode className="w-5 h-5" />
                       </div>
                       <div className="text-left">
                           <span className="text-sm font-bold block">Terminal</span>
                           <span className="text-[10px] text-blue-100 font-medium">Ler QR / Cobrar</span>
                       </div>
                   </button>

                   {/* 3. Promoções */}
                   <button onClick={() => onNavigate('weekly_promo')} className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col gap-3 items-start active:scale-95 transition-all">
                       ...
                       <div className="text-left">
                           <span className="text-sm font-bold text-gray-900 dark:text-white block">Promoções</span>
                           <span className="text-[10px] text-gray-500 dark:text-gray-400">Criar Ofertas</span>
                       </div>
                   </button>

                   {/* 4. Vagas */}
                   <button onClick={() => onNavigate('merchant_jobs')} className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col gap-3 items-start active:scale-95 transition-all">
                       ...
                       <div className="text-left">
                           <span className="text-sm font-bold text-gray-900 dark:text-white block">Vagas</span>
                           <span className="text-[10px] text-gray-500 dark:text-gray-400">Recrutamento</span>
                       </div>
                   </button>

                   {/* 5. Dados da Loja */}
                   <button onClick={() => onNavigate('store_profile')} className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col gap-3 items-start active:scale-95 transition-all">
                       ...
                       <div className="text-left">
                           <span className="text-sm font-bold text-gray-900 dark:text-white block">Perfil</span>
                           <span className="text-[10px] text-gray-500 dark:text-gray-400">Editar Dados</span>
                       </div>
                   </button>

                   {/* 6. Financeiro */}
                   <button onClick={() => onNavigate('store_finance')} className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col gap-3 items-start active:scale-95 transition-all">
                       ...
                       <div className="text-left">
                           <span className="text-sm font-bold text-gray-900 dark:text-white block">Financeiro</span>
                           <span className="text-[10px] text-gray-500 dark:text-gray-400">Faturas e Extrato</span>
                       </div>
                   </button>
               </div>
            </div>
           ...
           <div className="bg-white dark:bg-gray-900 w-full max-w-sm rounded-[2rem] overflow-hidden shadow-2xl relative border border-gray-100 dark:border-gray-800">
              ...
              <div className="p-6 text-center">
                 <h3 className="font-bold text-gray-900 dark:text-white mb-2">
                    {showVideoModal.type === 'merchant_jobs' ? 'Contrate no Bairro' :
                     showVideoModal.type === 'jobs' ? 'Entenda os alertas' : 'Economize com Cashback'}
                 </h3>
                 <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
                    {showVideoModal.type === 'merchant_jobs'
                      ? 'Veja como criar uma vaga e alcançar candidatos da região. Simplifique seu recrutamento agora.'
                 ...
                 >
                   {showVideoModal.type === 'merchant_jobs' ? 'Criar vaga agora' : 'Entendido!'}
                 </button>
              </div>
           </div>
        </div>
