...
interface StoreAreaViewProps {
  ...
  // FIX: User type is DbUser, not SupabaseUser
  user: DbUser | null;
}
...
export const StoreAreaView: React.FC<StoreAreaViewProps> = ({ onBack, onNavigate, user }) => {
  ...
  // FIX: Mock merchant ID for demo purposes
  const merchantId = user?.id || 'mock-merchant-id';
  const storeName = user?.full_name || STORE_DATA_MOCK.name; // Use user's full_name for store name if available
  const avatarUrl = user?.avatar_url || `https://ui-avatars.com/api/?name=${storeName.replace(' ', '+')}&background=1E5BFF&color=fff`;
  ...
  const currentKpis = useMemo(() => {
    ...
    return {
        sales: STORE_DATA_MOCK.baseKpis.sales * multiplier,
        orders: Math.round(STORE_DATA_MOCK.baseKpis.orders * multiplier),
        newCustomers: Math.round(STORE_DATA_MOCK.baseKpis.newCustomers * multiplier),
        recurringCustomers: Math.round(STORE_DATA_MOCK.baseKpis.recurringCustomers * multiplier),
        cashbackGiven: STORE_DATA_MOCK.baseKpis.cashbackGiven * multiplier,
        adBalance: STORE_DATA_MOCK.baseKpis.adBalance // Balance doesn't typically scale with time range view
    };
  }, [dateRange]);

  // Realtime Pending Requests Listener
  useEffect(() => {
    if (!supabase || !user) return; // FIX: Ensure user is available
  ...
  }, [merchantId, user]); // FIX: Add user to dependencies
        ...
        <div className="flex items-center gap-4">
            ...
            <div>
                <div className="flex items-center gap-1.5">
                    ...
                    {STORE_DATA_MOCK.isVerified && <BadgeCheck className="w-5 h-5 text-white fill-[#1E5BFF]" />}
                </div>
