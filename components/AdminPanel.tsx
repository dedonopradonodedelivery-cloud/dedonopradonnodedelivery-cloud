...
export const AdminPanel: React.FC<AdminPanelProps> = ({ user, onNavigateToApp, onLogout }) => {
  ...
  // Sync state with localStorage to allow cross-app simulation
  useEffect(() => {
    const savedMode = localStorage.getItem('admin_view_override') as RoleMode;
  ...
  // Componente de Header Reutilizável com Switcher
  const SharedHeader = ({ theme = 'dark' }: { theme?: 'dark' | 'light' }) => {
    const isDark = theme === 'dark';
    return (
      <header className={`px-6 py-8 flex items-center justify-between shrink-0 border-b transition-all ${
        isDark ? 'bg-[#111827] border-white/5 shadow-2xl' : 'bg-white border-slate-200'
      }`}>
        <div className="flex items-center gap-5">
          <div className={`w-14 h-14 flex items-center justify-center shadow-lg transition-colors ${
        ...
        <div className="flex items-center gap-4">
          <button 
              ...
              className={`px-4 py-2.5 rounded-xl flex items-center gap-3 border transition-all active:scale-95 ${
                isDark 
          ...
          >
              <div className="flex flex-col items-end">
                ...
                <span className="text-xs font-bold uppercase tracking-tight">{viewMode}</span>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
          ...
          >
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 font-bold leading-relaxed max-w-[240px] mx-auto">Monitoramento em tempo real da rede e gestão de infraestrutura.</p>
              </div>
  ...
  const renderMonetizationModel = () => (
    <div className="animate-in slide-in-from-right duration-500 space-y-12 bg-[#F8FAFC] min-h-screen pb-32 text-slate-900 -mx-6 px-6 pt-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white p-8 border border-slate-200 rounded-[2rem] shadow-sm space-y-6 col-span-1 md:col-span-2">
            <div className="flex justify-between items-start">
              <div>
                <h4 className="font-black text-[11px] text-slate-400 uppercase tracking-[0.2em] mb-2">01. Patrocinador Master</h4>
                <p className="text-xl font-bold text-slate-900 max-w-md leading-snug">Autoridade de marca e exclusividade institucional em 100% da rede.</p>
              </div>
              <span className="text-indigo-600 text-[9px] font-black uppercase tracking-widest bg-indigo-50 border border-indigo-100 px-3 py-1 rounded-lg">Exclusividade</span>
            </div>
            <div className="flex gap-12 items-end pt-4 border-t border-slate-100">
              <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1.5">Fee Mensal</p>
          ...
          <div className="bg-white p-8 border border-slate-200 rounded-[2rem] shadow-sm flex flex-col justify-between">
              <div>
                <h4 className="font-black text-[11px] text-slate-400 uppercase tracking-[0.2em] mb-3">02. Micro-Ads Bairro</h4>
                <p className="text-sm text-slate-500 leading-relaxed mb-8">Segmentação por CEP e comportamento regional.</p>
              </div>
          ...
          <div className="bg-white p-8 border border-slate-200 rounded-[2rem] shadow-sm flex flex-col justify-between">
              <div>
                <h4 className="font-black text-[11px] text-slate-400 uppercase tracking-[0.2em] mb-3">03. Lead Generation</h4>
                <p className="text-sm text-slate-500 leading-relaxed mb-8">Conversão direta de serviço para profissional verificado.</p>
              </div>
              <div className="flex justify-between items-end">
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] font-sans animate-in fade-in duration-500 flex flex-col overflow-x-hidden text-white relative">
      ...
      {/* Persistência de Header Independente da View */}
      <SharedHeader theme={activeView === 'monetization_model' ? 'light' : 'dark'} />

      <main className={`p-6 max-w-5xl mx-auto w-full flex-1 ${activeView === 'monetization_model' ? 'bg-[#F8FAFC]' : ''}`}>
      ...
      {isRoleSheetOpen && (
        <div className="fixed inset-0 z-[200] flex items-end justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
            <div 
                className="bg-[#111827] w-full max-w-md rounded-t-[2.5rem] border-t border-white/10 p-6 pb-12 shadow-2xl animate-in slide-in-from-bottom duration-500 overflow-hidden relative"
            ...
            >
                ...
                <div className="flex justify-between items-center mb-8 px-2">
                    <div>
                        ...
                        <p className="text-[10px] text-[#9CA3AF] font-black uppercase tracking-[0.2em] mt-1">Simular experiência de usuário</p>
                    </div>
                    <button onClick={() => setIsRoleSheetOpen(false)} className="w-10 h-10 bg-white/5 rounded-full flex items-center justify-center text-gray-500 hover:text-white transition-colors">
                        <X size={20} />
                    </button>
                </div>
                    ...
                    {roles.map((role) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {isSelected && (
                                    <div className="w-6 h-6 bg-[#059669] rounded-full flex items-center justify-center shadow-lg">
