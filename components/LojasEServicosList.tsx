...
export const LojasEServicosList: React.FC<LojasEServicosListProps> = ({ 
  ...
  const displayedStores = useMemo(() => {
    return limit ? filteredStores.slice(0, limit) : filteredStores.slice(0, page * 10);
  }, [filteredStores, limit, page]);

  const hasMore = !limit && displayedStores.length < filteredStores.length;
