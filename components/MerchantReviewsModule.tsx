...
export const MerchantReviewsModule: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const handleSendReport = () => {
    ...
    setReviews(prev => prev.map(r => r.id === reportingReview.id ? { ...r, status: 'moderation', isReported: true, reportReason, isHidden: true } : r));
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] dark:bg-gray-950 font-sans animate-in fade-in duration-300">
      <header className="sticky top-0 z-40 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md px-5 h-20 flex items-center gap-4 border-b border-gray-100 dark:border-gray-800 shadow-sm">
        ...
        <div><h1 className="font-black text-xl text-gray-900 dark:text-white uppercase tracking-tighter leading-none">Avaliações</h1><p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1">Gerencie sua reputação local</p></div>
      </header>
      ...
      <main className="p-5 pb-32 space-y-8">
        ...
        <section className="space-y-4">
            <div className="flex bg-[#F1F2F4] dark:bg-gray-900 p-1 rounded-2xl">
        ...
        <section className="space-y-4">
            {filteredReviews.map((rev) => (
                <div key={rev.id} className={`bg-white dark:bg-gray-900 rounded-[2.5rem] border transition-all overflow-hidden ${rev.isReported ? 'border-red-100 dark:border-red-900/30' : 'border-gray-100 dark:border-gray-800'}`}>
                    <div className="p-6">
                        <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-full bg-[#F1F2F4] dark:bg-gray-800 flex items-center justify-center text-gray-400"><UserIcon size={20}/></div>
                                <div>
                        ...
                        {rev.response && (
                            <div className="mt-4 p-4 bg-[#F1F2F4] dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800">
                        ...
                        {!respondingTo || respondingTo !== rev.id ? (
                            <div className="mt-6 flex gap-3 pt-4 border-t border-gray-50 dark:border-gray-800">
                                ...
                                <button onClick={() => setReportingReview(rev)} className="p-3 bg-[#F1F2F4] dark:bg-gray-800 text-gray-400 rounded-xl hover:text-red-500 transition-colors"><Flag size={18} /></button>
                            </div>
                        ) : (
                            <div className="mt-6 animate-in slide-in-from-bottom-2 duration-300">
                                <textarea autoFocus value={responseText} onChange={(e) => setResponseText(e.target.value)} placeholder="Escreva sua resposta profissional..." className="w-full p-4 bg-[#F1F2F4] dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-blue-500/20 dark:text-white transition-all h-24 resize-none" />
