...
export const StoreAdsModule: React.FC<StoreAdsModuleProps> = ({ onBack }) => {
        ...
        {campaigns.length > 0 && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center justify-between mb-4 px-1">
              ...
              <span className="text-[10px] text-[#1E5BFF] dark:text-purple-400 font-black bg-blue-500/10 px-3 py-1 rounded-full border border-blue-500/20">
  ...
  const CreateView = () => (
    <div className="flex flex-col min-h-screen bg-white dark:bg-slate-950 transition-colors duration-300">
      {/* Stepper */}
      <div className="p-5 flex justify-between mb-2 px-8 relative">
        <div className="absolute top-9 left-0 right-0 h-0.5 bg-gray-100 dark:bg-slate-800 -z-0 mx-12"></div>
        {[1, 2, 3].map(step => (
          <div key={step} className="flex flex-col items-center gap-2 z-10 bg-white dark:bg-slate-950 px-2">
            ...
            <span className={`text-[9px] font-black uppercase tracking-wider ${createStep >= step ? 'text-[#1E5BFF] dark:text-purple-500' : 'text-slate-400 dark:text-slate-600'}`}>
      ...
      <div className="flex-1 p-5 pb-40 overflow-y-auto no-scrollbar">
            ...
            <button 
              ...
              className={`w-full p-6 rounded-3xl border-2 text-left transition-all group ${
                newCampaignType === 'local' 
                  ...
                  : 'border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-slate-900/50 hover:bg-white dark:hover:bg-slate-900'
              }`}
            >
              ...
              <p className="text-xs text-gray-500 dark:text-slate-400 leading-relaxed font-medium pl-13">
        ...
        {createStep === 2 && (
          <div className="space-y-6 animate-in slide-in-from-right duration-300 w-full max-w-full">
            ...
            <div className="bg-gray-50 dark:bg-slate-900 p-6 rounded-3xl border border-gray-100 dark:border-white/5 w-full">
              <div className="flex justify-between items-center mb-6">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Duração da campanha</label>
                <span className="text-lg font-black text-[#1E5BFF] dark:text-purple-500 px-3 py-1 bg-blue-500/10 rounded-xl border border-blue-500/20">
                  {duration} dias
                </span>
              </div>
              
              <div className="px-2 relative mb-2">
                <div className="absolute left-0 right-0 h-1 top-1/2 -translate-y-1/2 flex justify-between pointer-events-none px-2">
                    <div className="w-1 h-3 bg-gray-300 dark:bg-slate-700 rounded-full"></div>
                    <div className="w-1 h-3 bg-gray-300 dark:bg-slate-700 rounded-full"></div>
                </div>
                ...
                <input 
                  ...
                  className="w-full h-2 bg-gray-200 dark:bg-slate-800 rounded-lg appearance-none cursor-pointer accent-[#1E5BFF] dark:accent-purple-600 relative z-10 custom-slider"
                />
              </div>
              
              <div className="flex justify-between px-1 text-[10px] font-bold text-gray-400 dark:text-slate-600 uppercase tracking-tighter">
                <span>15 dias</span>
                <span>30 dias</span>
              </div>
              
              <p className="text-center text-[10px] text-gray-400 dark:text-slate-500 font-medium mt-6 italic uppercase tracking-widest">
            ...
            <div className="bg-gradient-to-r from-blue-600/10 to-indigo-600/10 dark:from-purple-600/20 dark:to-indigo-600/20 p-6 rounded-3xl flex justify-between items-center border border-blue-500/20 dark:border-white/10 shadow-sm w-full">
               <div>
                    <span className="text-[10px] font-black text-[#1E5BFF] dark:text-purple-300 uppercase tracking-widest block mb-1">Investimento Estimado</span>
                    <span className="text-xs text-gray-500 dark:text-slate-500 font-bold">Total: {duration} dias × R$ {currentPrice.toFixed(2).replace('.', ',')}</span>
               </div>
          ...
          <div className="space-y-6 animate-in slide-in-from-right duration-300 w-full">
            <div className="bg-gray-50 dark:bg-slate-900 p-8 rounded-[32px] border border-gray-100 dark:border-white/5 shadow-2xl relative overflow-hidden">
                ...
                <div className="space-y-4 mb-8 text-sm">
                    ...
                    <div className="flex justify-between items-center border-b border-gray-200 dark:border-white/5 pb-4">
                        <span className="text-slate-500 font-medium">Valor Diário</span>
                        <span className="font-bold text-gray-900 dark:text-white">R$ {currentPrice.toFixed(2).replace('.', ',')}</span>
                    </div>
                    <div className="flex justify-between items-center pt-2">
                        <span className="font-black text-slate-400">Total a Pagar</span>
                ...
                <div className={`p-5 rounded-2xl border flex items-start gap-4 transition-all ${hasSufficientBalance ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-orange-500/10 border-orange-500/20'}`}>
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${hasSufficientBalance ? 'bg-emerald-500/20 text-emerald-600' : 'bg-orange-500/20 text-orange-600'}`}>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-5 bg-white/90 dark:bg-slate-950/80 backdrop-blur-xl border-t border-gray-100 dark:border-white/5 z-30 flex gap-3 max-w-md mx-auto">
        <button 
          onClick={() => createStep === 1 ? setView('list') : setCreateStep(prev => prev - 1)}
          className="flex-1 py-4 rounded-2xl border border-gray-200 dark:border-white/10 text-slate-400 font-black text-xs uppercase tracking-widest active:scale-95 transition-all"
        >
        ...
        {createStep > 1 && (
          <button 
            ...
            className={`flex-[2] bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-black text-xs uppercase tracking-widest py-4 rounded-2xl shadow-xl active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-30 disabled:grayscale`}
          >
            {isActivating ? (
      ...
      {isActivating && (
        <div className="fixed inset-0 z-[100] bg-white/90 dark:bg-slate-950/90 backdrop-blur-sm flex flex-col items-center justify-center text-center p-8 animate-in fade-in duration-300">
            ...
            <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-2 font-display">Ativando ADS</h2>
            <p className="text-gray-500 dark:text-slate-500 text-sm max-w-[240px]">Estamos configurando sua visibilidade premium na Freguesia...</p>
        </div>
      )}
  ...
  const DetailsView = () => {
    ...
    return (
      <div className="p-5 pb-32 animate-in slide-in-from-right duration-300 bg-white dark:bg-slate-950 min-h-screen transition-colors">
        <div className="bg-gray-50 dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-white/5 mb-6">
            <div className="flex justify-between items-start mb-6">
                <div>
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white leading-tight mb-1">{selectedCampaign.name}</h2>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl border border-gray-100 dark:border-white/5">
                    ...
                    <p className="text-2xl font-black text-blue-600">{selectedCampaign.metrics.clicks}</p>
                </div>
                <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl border border-gray-100 dark:border-white/5">
        ...
        <h3 className="font-bold text-gray-900 dark:text-white mb-4 px-1 flex items-center gap-2 uppercase text-xs tracking-widest opacity-60">
            ...
            <div className="flex items-end justify-between h-32 gap-3">
                {selectedCampaign.history.map((val, i) => (
                    <div key={i} className="flex-1 flex flex-col justify-end items-center gap-2">
                        ...
                        ></div>
                        <span className="text-[8px] text-slate-400 dark:text-slate-600 font-black uppercase">{['D', 'S', 'T', 'Q', 'Q', 'S', 'S'][i]}</span>
                    </div>
                ))}
            </div>
            <div className="mt-8 p-4 bg-white dark:bg-slate-950 rounded-2xl border border-gray-100 dark:border-white/5 text-center shadow-inner">
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 font-sans flex flex-col transition-colors duration-300">
      <div className="sticky top-0 z-30 bg-white/90 dark:bg-slate-950/90 backdrop-blur-md px-5 h-16 flex items-center gap-4 border-b border-gray-100 dark:border-white/5">
        <button 
