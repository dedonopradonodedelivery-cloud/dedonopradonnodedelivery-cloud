        ...
        <button onClick={handleFooterClick} disabled={isSubmitting} className={`w-full py-5 rounded-[2rem] shadow-xl shadow-blue-500/20 flex flex-col items-center justify-center transition-all active:scale-[0.98] ${selectedMode ? 'bg-[#1E5BFF] text-white' : 'bg-white/5 text-slate-500 cursor-not-allowed opacity-50'}`}>
          {isSubmitting ? <Loader2 className="w-6 h-6 animate-spin" /> : !selectedMode ? <span className="font-black text-sm uppercase tracking-widest">Escolha onde aparecer</span> : (
              <div className="flex flex-col items-center">
                  <div className="flex items-center gap-2 mb-0.5"><span className="text-[10px] font-black text-white/60 uppercase tracking-widest">ANUNCIAR: {selectedMode.label}</span><ArrowRight size={14} className="text-white/60" /></div>
                  <div className="flex items-baseline gap-2">
                    {!prices.isPackage && <span className="text-xs font-bold text-white/40 line-through">R$ {prices.original.toFixed(2)}</span>}
                    <span className="text-xl font-black text-white">R$ {prices.current.toFixed(2)}</span>
                  </div>
                  {prices.isPackage && (
                    <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mt-1">3x R$ {prices.monthly.toFixed(2)} sem juros</span>
                  )}
              </div>
          )}
        </button>
