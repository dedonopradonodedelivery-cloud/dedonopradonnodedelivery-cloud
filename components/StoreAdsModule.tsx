...
import { 
  ...
  QrCode,
  Info,
  // Added Clock to fix the reference error on line 319
  Clock
} from 'lucide-react';
...
const MOCK_INVENTORY: Record<string, any> = {
  ...
  "Taquara": { "Março": { home: 1, cat: 0 }, "Abril": { home: 2, pointer_events_none: 2 } },
  "Anil": { "Março": { home: 0, cat: 0 }, "Abril": { home: 1, cat: 1 } },
};

const DISPLAY_MODES = [
  { id: 'home', label: 'Home', icon: Home },
  { id: 'cat', label: 'Categorias', icon: LayoutGrid },
  { id: 'combo', label: 'Home + Cat', icon: Zap, recommended: true },
];
...
export const StoreAdsModule: React.FC<StoreAdsModuleProps> = ({ onBack, onNavigate, user, categoryName }) => {
  const [selectedMode, setSelectedMode] = useState<'home' | 'cat' | 'combo' | null>(null);
  ...
  const getAvailability = (hood: string, month: string) => {
    ...
    return Math.min(data.home || 0, data.cat || 0);
  };
    ...
    return (
        <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center p-6 text-center animate-in zoom-in duration-500">
            ...
            <p className="text-slate-400 text-sm max-w-xs mx-auto leading-relaxed">
                {artChoice === 'pro' 
                    ? 'Pagamento em análise. Você será chamado no chat em alguns minutos para iniciarmos sua arte profissional.' 
                    : 'Pagamento em análise. Seu banner entrará no ar assim que o pagamento for compensado.'}
            </p>
      ...
      <header className="sticky top-0 z-50 bg-[#020617]/80 backdrop-blur-xl border-b border-white/5 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-blue-400 uppercase font-black tracking-widest mt-1">Configuração em Página Única</p>
          </div>
        </div>
      </header>

      <main className="flex-1 p-6 space-y-16 pb-60 max-w-md mx-auto w-full">
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            {DISPLAY_MODES.map((mode) => (
              <button
                key={mode.id}
                onClick={() => { setSelectedMode(mode.id as any); scrollTo(neighborhoodRef); }}
          ...
          <div className="space-y-5">
            <div className="flex justify-between items-end">
                <div>
                    ...
                    <p className="text-[9px] text-slate-500 uppercase font-bold tracking-widest ml-1 mt-1">Disponibilidade em tempo real</p>
                </div>
                <button 
                    onClick={() => {
                        const allAvail = NEIGHBORHOODS.filter(h => getAvailability(h, selectedMonth) > 0);
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-2">
                                {avail === 0 && nextMonth && (
                                    <span className="text-[8px] font-black text-blue-400 uppercase border border-blue-400/20 px-2 py-1 rounded-md">Vaga em {nextMonth}</span>
                                )}
                                {avail > 0 && (
        ...
        {/* BLOCO 3: CRIAÇÃO DA ARTE (PROGRESSIVO) */}
        <section 
          ref={creativeRef}
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-5">
              {/* DIY CHOICE */}
              <div className={`rounded-[2.5rem] border-2 transition-all overflow-hidden ${artChoice === 'diy' ? 'bg-slate-800 border-blue-500' : 'bg-slate-900 border-white/5 opacity-80'}`}>
                <button 
                    onClick={() => { setArtChoice('diy'); scrollTo(paymentRef); }}
                    className="w-full p-8 text-left flex items-start gap-5"
                >
                ...
                {artChoice === 'diy' && (
                    <div className="px-8 pb-8 pt-2 animate-in slide-in-from-top-4 duration-300">
                         <div className="w-full aspect-[21/9] rounded-2xl bg-blue-600 flex flex-col items-center justify-center text-center p-4 mb-6 shadow-2xl">
                            <h4 className="font-black text-xl text-white leading-tight">{diyConfig.title}</h4>
                            <p className="text-[9px] text-white/80 font-medium">{diyConfig.description}</p>
                        </div>
                        <input value={diyConfig.title} onChange={e => setDiyConfig({...diyConfig, title: e.target.value})} className="w-full bg-slate-950 border border-white/5 rounded-xl p-4 text-xs font-bold text-white mb-3" placeholder="Título chamativo" />
                        <button onClick={() => scrollTo(paymentRef)} className="text-[10px] font-black text-blue-400 uppercase tracking-widest flex items-center gap-2">Finalizar Arte <ArrowRight size={12}/></button>
                    </div>
                )}
              </div>
              ...
              >
                  ...
                  <div className="space-y-2 mb-8">
                    ...
                    <div className="flex items-center gap-3 text-slate-300"><Target size={14} className="text-amber-400" /><span className="text-[10px] font-bold">Design focado em conversão local</span></div>
                  </div>
        ...
        >
            ...
            <div className="bg-slate-900 rounded-[2.5rem] p-8 border border-white/10 shadow-2xl space-y-6">
                <div className="space-y-3">
                    <div className="flex justify-between text-sm"><span className="text-slate-500">Onde aparece</span><span className="font-bold text-white uppercase">{selectedMode === 'combo' ? 'Home + Categoria' : selectedMode}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-slate-500">Período</span><span className="font-bold text-white">{selectedMonth}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-slate-500">Bairros</span><span className="font-bold text-white text-right max-w-[180px] truncate">{selectedNeighborhoods.join(', ')}</span></div>
                    <div className="flex justify-between items-center py-2 border-t border-white/5 mt-2">
                        <span className="text-slate-500 text-sm">Design da Arte</span>
                        <span className="text-[10px] font-black bg-blue-500/10 text-blue-400 px-3 py-1 rounded-lg border border-blue-500/20 uppercase">{artChoice === 'pro' ? 'Time Localizei' : 'Personalizada'}</span>
                    </div>
                </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-[#020617]/95 backdrop-blur-2xl border-t border-white/10 z-40 max-w-md mx-auto shadow-[0_-20px_40px_rgba(0,0,0,0.4)]">
        <div className="flex justify-between items-end mb-6">
            <div className="space-y-1">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Investimento Final</p>
