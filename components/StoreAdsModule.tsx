...
export const StoreAdsModule: React.FC<StoreAdsModuleProps> = ({ onBack, onNavigate, user, categoryName, viewMode, initialView = 'sales' }) => {
  ...
  // AJUSTE 4: Lógica de Datas Sequenciais
  const getMonthDateRange = (index: number) => {
    const start = new Date();
    // Cada bloco tem 30 dias. O início do bloco 'index' é (index * 30) dias após hoje + 1 dia de ajuste se não for o primeiro
    ...
    return { start: fmt(start), end: fmt(end), rawStart: start, rawEnd: end };
  };
  ...
  const prices = useMemo(() => {
    ...
    // Mesmo sem meses ou bairros, o Passo 1 já compõe o valor base mensal
    ...
    // Cálculo do range total para o resumo
    let dateStart = '-';
    let dateEnd = '-';
    if (numMonths > 0) {
  ...
  const handleFinalize = () => {
    ...
    setTimeout(() => {
      ...
      badgeExpiry.setFullYear(now.getFullYear() + 1);
      const badgeData = { type: 'founder', status: 'active', expiryDate: badgeExpiry.toISOString() };
      const savedBadges = JSON.parse(localStorage.getItem('localizei_merchant_badges') || '{}');
      savedBadges[user.id] = badgeData;
      localStorage.setItem('localizei_merchant_badges', JSON.stringify(savedBadges));

      const totalDays = selectedMonths.length * 30;
      const campaignExpiry = new Date();
      campaignExpiry.setDate(now.getDate() + totalDays);
      const newOccupancy = selectedNeighborhoods.map(hood => ({
        hood, modeId: selectedMode.id, expiryDate: campaignExpiry.toISOString(), merchantId: user.id
      }));
      const currentOccupancy = JSON.parse(localStorage.getItem('localizei_hood_occupancy') || '[]');
      localStorage.setItem('localizei_hood_occupancy', JSON.stringify([...currentOccupancy, ...newOccupancy]));
  ...
  if (isSuccess) {
    return (
      <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center p-8 text-center animate-in zoom-in duration-500">
        ...
        <p className="text-slate-400 text-sm mb-2 max-w-xs mx-auto leading-relaxed">Sua campanha foi ativada e o bairro foi bloqueado para exclusividade.</p>
        <div className="bg-blue-500/10 border border-blue-500/30 px-4 py-2 rounded-xl mb-12 flex items-center gap-2"><Gem size={14} className="text-yellow-400" /><span className="text-[10px] font-black text-white uppercase tracking-widest">Selo Fundador Ativado</span></div>
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-100 font-sans flex flex-col selection:bg-blue-500/30">
      ...
      <main className="flex-1 w-full space-y-16 pb-[320px] overflow-y-auto no-scrollbar">
          ...
          {/* AJUSTE 1: ALINHAMENTO DO TOPO CENTRALIZADO */}
          <section className="px-6 pt-10 space-y-4 text-center flex flex-col items-center">
              ...
              {/* AJUSTE 2: REESCRITA DO BANNER FUNDADOR */}
              <div className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 border border-blue-500/30 p-6 rounded-[2.5rem] flex flex-col items-center gap-4 shadow-2xl relative overflow-hidden w-full text-center">
                  ...
                  <div className="relative z-10">
                      <h4 className="font-black text-white text-base uppercase tracking-tighter flex items-center justify-center gap-2">
                        💎 FUNDADOR APOIADOR
                      </h4>
                      <p className="text-[11px] text-white font-bold uppercase tracking-widest mt-1 leading-tight">
                        <span className="text-yellow-400">CONDIÇÃO ESPECIAL DE LANÇAMENTO</span>
                      </p>
                      <p className="text-[10px] text-slate-300 mt-4 leading-relaxed px-2">
                        Durante este período, apenas alguns lojistas terão acesso a um <span className="text-yellow-400">VALOR REDUZIDO</span> que será encerrado em breve.
                        <br/><br/>
                        Além do preço diferenciado, quem entra agora recebe um <span className="text-yellow-400">SELO EXCLUSIVO DE FUNDADOR APOIADOR</span>, visível no perfil da loja, garantindo destaque, autoridade e prioridade na plataforma.
                      </p>
                  </div>
              </div>
          </section>
      ...
      {/* AJUSTE 3 e 5: RESUMO DO PEDIDO E CTA UNIFICADOS E FIXOS ACIMA DA NAV */}
      <div className="fixed bottom-[80px] left-0 right-0 p-5 bg-slate-900/95 backdrop-blur-xl border-t border-white/10 z-[101] max-w-md mx-auto shadow-2xl flex flex-col gap-4">
          ...
          {/* AJUSTE 5: CTA FINAL OBRIGATÓRIAMENTE VISÍVEL */}
          <button 
