...
export const HomeFeed: React.FC<HomeFeedProps> = ({ 
  ...
  return (
    <div className="flex flex-col bg-white dark:bg-gray-950 w-full max-w-md mx-auto animate-in fade-in duration-500 overflow-x-hidden pb-32">
        ...
        <section className="w-full bg-white dark:bg-gray-950 pt-4 pb-2 border-b border-gray-50 dark:border-gray-900 overflow-hidden">
            <div className="flex items-center gap-3 overflow-x-auto no-scrollbar px-5 flex-nowrap">
                {topCategories.map((cat) => (
                    <button 
                        key={cat.id} 
                        onClick={() => trackCategoryClick(cat)} 
                    ...
                    >
                        ...
                        <span className="text-[8px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-tighter text-center truncate w-14">
                            {cat.name}
                        </span>
                    </button>
                ))}
      ...
      {/* 2) CARROSSEL UNIFICADO (SERVIÇOS + COMERCIAIS) */}
      {isFeatureActive('banner_highlights') && (
        <section className="bg-white dark:bg-gray-950 w-full pt-2">
