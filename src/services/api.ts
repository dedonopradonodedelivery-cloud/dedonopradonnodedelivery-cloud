...
export const validateStoreCode = async (code: string) => {
  if (!supabase) {
    if (code.toUpperCase().startsWith('JPA-') || code.length > 20) {
      return {
        id: 'lojista-demo-id', name: 'Estabelecimento Parceiro (Demo)', category: 'Geral',
        merchant_cashback_settings: { cashback_percent: 5, validity_days: 30 }
      };
    }
  ...
  const { data, error } = await supabase
    .from('merchants').select(`id, name, category, is_active, merchant_cashback_settings (cashback_percent, validity_days)`)
    .or(`secure_id.eq.${code},manual_code.eq.${cleanCode}`).maybeSingle();

  if (error) { console.error("Database validation error:", error); throw new Error("Erro técnico na validação. Tente novamente."); }
  if (!data) { throw new Error("Código não reconhecido. Peça ao lojista o código atualizado."); }
  if (!data.is_active) { throw new Error("Este estabelecimento não está aceitando transações no momento."); }
  return data;
};
...
export const initiateTransaction = async (params: { userId: string; storeId: string; merchantId: string; amountCents: number; type: 'earn' | 'use'; purchaseTotalCents: number; }) => {
    ...
    const { data, error } = await supabase.from('cashback_transactions').insert({
        user_id: params.userId, store_id: params.storeId, merchant_id: params.merchantId,
        purchase_total_cents: params.purchaseTotalCents, amount_cents: params.amountCents,
...
export const getPendingTransactions = async (merchantId: string) => {
  if (!supabase) return [];
  const { data, error } = await supabase.from('cashback_transactions').select(`*, users ( name )`).eq('merchant_id', merchantId).eq('status', 'pending').order('created_at', { ascending: false });
...
export const getUserWallet = async (userId: string) => {
  ...
  const { data: user, error: userError } = await supabase.from('users').select('wallet_balance').eq('id', userId).single();
  ...
  return { balance: user.wallet_balance, history: history as DbWalletMovement[] };
};
