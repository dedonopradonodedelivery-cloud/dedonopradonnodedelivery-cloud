...
const HighlightBanner: React.FC<{ banner: BannerAd; onClick: (id: string) => void; }> = ({ banner, onClick }) => (
  <div onClick={() => onClick(banner.id)} className="snap-center flex-shrink-0 w-full h-[160px] rounded-[20px] shadow-lg shadow-black/5 overflow-hidden relative group cursor-pointer select-none">
    <img src={banner.image} alt={banner.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ease-in-out pointer-events-none" />
...
const SponsoredCarousel: React.FC<{ banners: BannerAd[]; onView?: (id: string) => void; onClick?: (id: string) => void; }> = ({ banners, onView = () => {}, onClick = () => {} }) => {
  ...
  }, [currentSlide, banners.length, isPaused]);
  ...
  return (
    <div 
        ...
        onMouseLeave={resume}
    >
      <div ref={scrollRef} onScroll={handleScroll} className="flex gap-4 overflow-x-auto snap-x snap-mandatory no-scrollbar touch-pan-x">
