...
const TemplateBannerRender: React.FC<{ config: any }> = ({ config }) => {
    ...
    switch (template_id) {
      case 'oferta_relampago':
        return (
          <div className="w-full h-full rounded-2xl bg-gradient-to-br from-rose-500 to-red-600 text-white p-6 flex items-center justify-between overflow-hidden relative shadow-lg select-none">
      ...
      case 'lancamento':
        return (
          <div className="w-full h-full rounded-2xl bg-gradient-to-br from-slate-800 to-slate-900 text-white p-6 flex items-end justify-between overflow-hidden relative shadow-lg select-none">
             <img src={product_image_url || 'https://via.placeholder.com/150'} className="absolute inset-0 w-full h-full object-cover opacity-30 mix-blend-luminosity pointer-events-none" />
...
const CustomBannerRender: React.FC<{ config: any }> = ({ config }) => {
    ...
    return (
        <div 
            className={`w-full h-full rounded-2xl overflow-hidden relative shadow-lg p-8 ${layoutClasses[template_id] || 'flex flex-col justify-center'} select-none`}
...
export const CategoryView: React.FC<CategoryViewProps> = ({ category, onBack, onStoreClick, stores, userRole, onAdvertiseInCategory, onNavigate }) => {
    ...
    const fetchCategoryBanners = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .limit(5); // Pega até 5 banners

        if (error) throw error;
        setBanners(data || []);
      } catch (e: any) {
        console.error("Failed to fetch category banners from Supabase:", e.message || e);
        setBanners([]);
      } finally {
        setLoadingBanner(false);
      }
  ...
  }, [activeBanner, category.name, currentNeighborhood]);
        ...
        <section>
          {loadingBanner ? (
            ...
            <div className="relative group" {...handlers}>
                <div 
                    className="w-full aspect-video touch-pan-y"
                    onClick={() => {
                        trackAdEvent(
                            'ad_click',
                            activeBanner.id,
                ...
                {/* Indicadores de Progresso se houver mais de 1 */}
                {banners.length > 1 && (
                    <div className="flex gap-1.5 absolute bottom-4 left-1/2 -translate-x-1/2 z-20">
                        {banners.map((_, idx) => (
