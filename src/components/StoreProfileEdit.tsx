      ...
      <div className="p-6 space-y-12 max-w-md mx-auto">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] p-8 border border-blue-50 dark:border-gray-800 shadow-sm space-y-4">
             <div className="grid grid-cols-2 gap-3">
                 {PAYMENT_METHODS.map(m => (
                     <div key={m.id} onClick={() => {}} className={`p-4 rounded-2xl border-2 flex items-center gap-3 cursor-pointer transition-all ${'bg-blue-50 border-blue-600'}`}>
                        <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${'bg-blue-600 border-blue-600'}`}>
        ...
        <section className="space-y-6">
          ...
          <div className="bg-slate-50 dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
             ...
             <FormField label="Razão Social" value={formData.razao_social} onChange={v => setFormData({...formData, razao_social: v.toUpperCase()})} placeholder="RAZÃO SOCIAL DA EMPRESA LTDA" />
             <FormField label="CNPJ" value={formData.cnpj} onChange={v => setFormData({...formData, cnpj: v})} placeholder="00.000.000/0001-00" />
             <FormField label="E-mail Fiscal / Faturamento" value={formData.email_fiscal} onChange={v => setFormData({...formData, email_fiscal: v})} placeholder="financeiro@empresa.com.br" />
          </div>
        </section>
