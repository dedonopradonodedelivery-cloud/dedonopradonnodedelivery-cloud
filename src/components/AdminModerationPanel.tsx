...
export const AdminModerationPanel: React.FC<AdminModerationPanelProps> = ({ onBack }) => {
  ...
  const handleTaxonomyModerate = (id: string, action: 'approve' | 'reject') => {
    ...
    if (action === 'approve') {
      ...
      sendNotification(sug.merchantId, 'Sugestão Recusada', `Não pudemos aprovar "${sug.name}". Motivo: ${rejectionReason}`, 'claim_rejection'); // FIX: Changed type to 'claim_rejection'
