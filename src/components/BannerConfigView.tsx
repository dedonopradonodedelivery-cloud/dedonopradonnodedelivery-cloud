...
const calculateBannerPrice = (
  ...
  neighborhoodsCount: number
    ...
    if (placement === 'Todos') {
        basePricePerMonth = BANNER_BASE_PRICES_CENTS['home'][duration] + BANNER_BASE_PRICES_CENTS['categorias'][duration];
    } else {
        basePricePerMonth = BANNER_BASE_PRICES_CENTS[placement.toLowerCase() as 'home' | 'categorias'][duration];
    }
...
export const BannerConfigView: React.FC<BannerConfigViewProps> = ({ onBack, onConfigure }) => {
    ...
    const savings = useMemo(() => {
        ...
        if (placement === 'Todos') {
             price1m = BANNER_BASE_PRICES_CENTS.home['1m'] + BANNER_BASE_PRICES_CENTS.categorias['1m'];
             ...
             price1m = BANNER_BASE_PRICES_CENTS[key]['1m'];
