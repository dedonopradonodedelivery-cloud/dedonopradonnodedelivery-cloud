...
export const App: React.FC = () => {
  ...
  const handlePaymentComplete = (paymentMethod: 'pix' | 'credit' | 'debit') => {
      ...
      if (isProfessionalService) {
          ...
          const msg1: BannerMessage = { id: `msg-sys-1-${Date.now()}`, orderId: newOrderId, senderType: 'system', body: '✅ Pagamento confirmado! Seu plano de anúncio já está reservado e agora vamos criar seu banner profissional.', createdAt: now };
          const msg2: BannerMessage = { id: `msg-sys-2-${Date.now() + 10}`, orderId: newOrderId, senderType: 'system', type: 'form_request', body: 'Para começar, precisamos de algumas informações. Por favor, preencha o formulário abaixo com seu logo e textos.', createdAt: new Date(Date.now() + 1000).toISOString() };
  ...
  const handleConfirmSponsoredPayment = () => setActiveTab('sponsored_ads_success');
  const handleCompleteSponsoredFlow = () => { setSponsoredPlan(null); setActiveTab('store_area'); };
    ...
    setBannerOrders(prev => prev.map(o => {
        if (o.id === orderId) {
            ...
            if (updates.onboardingStage === 'assets_received' && !o.autoMessagesFlags.assetsReceivedSent) {
                const autoMsg: BannerMessage = { id: `msg-sys-${Date.now()}-assets-ack`, orderId: orderId, senderType: 'system', body: 'Recebemos seus arquivos! Nossa equipe de design já está trabalhando na sua arte. Em breve enviaremos um rascunho.', createdAt: new Date().toISOString() };
  ...
  const handleViewOrder = (orderId: string) => {
    const now = new Date().toISOString();
    setBannerOrders(prev => prev.map(order => order.id === orderId ? { ...order, lastViewedAt: now } : order));
  ...
  const handleAdminViewOrder = (orderId: string) => { setAdminViewOrderId(orderId); setActiveTab('admin_banner_order_detail'); };

  useEffect(() => {
    if (activeTab === 'banner_order_tracking') {
      if (!viewingOrderId) { setActiveTab('store_area'); return; }
      const order = bannerOrders.find(o => o.id === viewingOrderId);
      if (!order || order.paymentStatus !== 'paid') setActiveTab('store_area');
    }
    ...
    return (
      <div className={`fixed inset-0 z-[999] flex flex-col items-center justify-between py-24 transition-all duration-800 ${splashStage === 3 ? 'animate-app-exit' : ''}`} style={{ backgroundColor: '#1E5BFF' }}>
        ...
        <div className="flex flex-col items-center animate-fade-in opacity-0" style={{ animationDelay: '3000ms', animationFillMode: 'forwards' }}>
             <p className="text-[9px] font-black text-white/50 uppercase tracking-[0.2em] mb-1.5">Patrocinador Master</p>
  ...
  if (!isApiKeySelected) {
    return <ApiKeyRequiredModal onSelectApiKey={handleSelectApiKey} onClose={() => setIsApiKeySelected(true)} />;
  }
        ...
        <div className="min-h-screen bg-white dark:bg-gray-900 flex justify-center relative">
          <Layout activeTab={activeTab} setActiveTab={setActiveTab} userRole={userRole as 'cliente' | 'lojista' | null} hideNav={hideBottomNav} viewMode={viewMode}>
              ...
              <main className="animate-in fade-in duration-500 w-full max-w-md mx-auto">
                ...
                {activeTab === 'admin_banner_orders_list' && user?.email === ADMIN_EMAIL && <AdminBannerOrdersList orders={bannerOrders} messages={bannerMessages} onBack={() => setActiveTab('admin_panel')} onSelectOrder={handleAdminViewOrder} />}
                {activeTab === 'admin_banner_order_detail' && user?.email === ADMIN_EMAIL && adminViewOrderId && <AdminBannerOrderDetail orderId={adminViewOrderId} orders={bannerOrders} messages={bannerMessages} onBack={() => setActiveTab('admin_banner_orders_list')} onSendMessage={handleAdminSendMessage} onUpdateOrder={handleUpdateOrder} />}
                ...
                {activeTab === 'sponsored_ads_checkout' && sponsoredPlan && <SponsoredAdsCheckoutView plan={sponsoredPlan} onBack={() => setActiveTab('sponsored_ads')} onConfirmPayment={handleConfirmSponsoredPayment} />}
                {activeTab === 'sponsored_ads_success' && sponsoredPlan && <SponsoredAdsSuccessView plan={sponsoredPlan} onComplete={handleCompleteSponsoredFlow} />}
                {activeTab === 'banner_order_tracking' && viewingOrderId && <BannerOrderTrackingView orderId={viewingOrderId} orders={bannerOrders} messages={bannerMessages} onBack={() => { setViewingOrderId(null); setActiveTab('store_area'); }} onSendMessage={handleSendMessage} onViewOrder={handleViewOrder} onUpdateOrder={handleUpdateOrder} />}
              </main>
